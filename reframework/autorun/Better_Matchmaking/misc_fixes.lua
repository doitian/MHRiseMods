...
function misc_fixes.init_module()
	...
	--[[
	local session_manager_type_def = sdk.find_type_definition("snow.SnowSessionManager");
	local methods = session_manager_type_def:get_methods();

	for a, method in pairs(methods) do
		local method_name = method:get_name();
		if method_name == "get_sessionSearchResult"
		or method_name == "getLastSearchSessionInfo"
		or method_name == "get_sessionSearchKeyOverwriteResult"
		or method_name == "update"
		or method_name == "set_OnEnterHall"
		or method_name == "isInAnySession"
		or method_name == "isInSession"
		or method_name == "clearFriendInvitetion" then
			goto continue;
		end
		
		sdk.hook(method, function(args)
			if methods_table[method_name] == nil then
				methods_table[method_name] = 1;
			else
				methods_table[method_name] = methods_table[method_name] + 1;
			end

			xy = table_helpers.tostring(methods_table);
		end, function(retval)
			return retval;
		end);
